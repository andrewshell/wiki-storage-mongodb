  ...
  module.exports = exports = function(argv) {
    ...
    storage.copyFile = async function (sLoc, tLoc, cb) {
      try {
        ...
        const doc = await db.collection('wiki').findOne({ _id: sLoc });
        const query = { _id: tLoc };
        const update = { $set: { _id: tLoc, page: doc.page } };
        const options = { upsert: true };
        await db.collection('wiki').updateOne(query, update, options);
        return cb();
      } catch (err) {
        console.error(err);
        return cb(err);
    ...
    storage.exists = async function (loc, cb) {
      try {
        const db = await mongo.db(argv);
        const doc = await db.collection('wiki').findOne({ _id: loc });
        return cb(!!doc);
      } catch (err) {
        console.error(err);
        return cb(false);
    ...
    storage.readDir = async function (loc, cb) {
      try {
        const db = await mongo.db(argv);
        const docs = await db.collection('wiki').find({}).project({ _id: 1, page: 0 }).toArray();
    ...
    storage.rename = async function (sLoc, tLoc, cb) {
      try {
        const db = await mongo.db(argv);
        const doc = await db.collection('wiki').findOne({ _id: sLoc });
        await db.collection('wiki').deleteOne({ _id: sLoc });
        const query = { _id: tLoc };
        const update = { $set: { _id: tLoc, page: doc.page } };
        const options = { upsert: true };
        await db.collection('wiki').updateOne(query, update, options);
    ...
    storage.unlink = async function (loc, cb) {
      try {
        const db = await mongo.db(argv);
        await db.collection('wiki').deleteOne({ _id: loc });
        return cb();
      } catch (err) {
        console.error(err);
        return cb(err);
